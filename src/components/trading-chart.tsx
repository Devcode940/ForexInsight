...
export const TradingChart = forwardRef<TradingChartHandle, TradingChartProps>(({ data, indicators, signal, symbol, timeframe }, ref) => {
  ...
  const [manualLines, setManualLines] = useState<{ id: string; price: number }[]>([]);
  ...
  useImperativeHandle(ref, () => ({
    ...
    getVisibleData: () => {
      ...
      return data.slice(Math.max(0, Math.floor(logicalRange.from)), Math.min(data.length - 1, Math.ceil(logicalRange.to)) + 1);
    }
  }));
